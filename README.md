...
### 表现(小市值组合,2015–2025,诚实成交口径,毛于成本)

**多样本窗口** —— 夏普与最大回撤,择时前 → 后:

| 窗口 | 夏普(原→择时) | 最大回撤(原→择时) | 在市 |
|---|---|---|---|
| 全周期 2015–2025 | 1.06 → **2.22** | −55% → **−32%** | 34% |
| 样本内 2015–2020 | 1.03 → 2.79 | −55% → −21% | 36% |
| 样本外 2021–2025 | 1.07 → 1.48 | −52% → −32% | 32% |
| 近三年 2023–2025 | 0.94 → 1.29 | −52% → −32% | 33% |
| 近两年 2024–2025 | 0.79 → 1.26 | −52% → −32% | 34% |
| 近一年 2025 | 1.40 → **−0.26** | −19% → −24% | — |

**分市场状态** —— 日均收益(价值高度非对称,几乎全部来自熊市避险):
...
| 2015 股灾 | +2% / −55% | **+190% / −21%** |
| 2018 熊市 | −15% / −30% | **+38% / −11%** |
| 2024 微盘踩踏 | −46% / −52% | **+11% / −5%** |

预测力:早盘广度对成交后(09:55→收盘)市场收益 IC≈0.18(Newey–West t=8.1)。机理上是一个**广义的早盘情绪 beta 择时器**,对沪深300 等大盘同样减风险。一句话:**风险削减器**——熊市/踩踏大幅避险,牛市/震荡因踏空让利(2025 单边上行年为负),不是收益增强器。
...
### Performance (small-cap book, 2015–2025, execution-honest, gross of cost)

**Across sample windows** — Sharpe and max drawdown, untimed → timed:

| Window | Sharpe (raw → timed) | Max drawdown (raw → timed) | In-market |
|---|---|---|---|
| Full 2015–2025 | 1.06 → **2.22** | −55% → **−32%** | 34% |
| In-sample 2015–2020 | 1.03 → 2.79 | −55% → −21% | 36% |
| Out-of-sample 2021–2025 | 1.07 → 1.48 | −52% → −32% | 32% |
| Trailing 3Y 2023–2025 | 0.94 → 1.29 | −52% → −32% | 33% |
| Trailing 2Y 2024–2025 | 0.79 → 1.26 | −52% → −32% | 34% |
| Trailing 1Y 2025 | 1.40 → **−0.26** | −19% → −24% | — |

**By market regime** — mean daily return (sharply asymmetric; the value is almost all bear-market protection):

| Regime | Untimed | Timed |
|---|---|---|
| Bull | +0.211% | +0.148% |
| Sideways | +0.627% | +0.462% |
| Bear | −0.057% | **+0.155%** |

**Stress windows** — period return / max drawdown:

| Window | Untimed | Timed |
|---|---|---|
| 2015 crash | +2% / −55% | **+190% / −21%** |
| 2018 bear | −15% / −30% | **+38% / −11%** |
| 2024 micro cascade | −46% / −52% | **+11% / −5%** |

Predictive power: breadth forecasts the post-signal (09:55→close) market return with
IC ≈ 0.18 (Newey–West t = 8.1). Mechanistically it is a **broad early-session sentiment
beta timer** that de-risks even large-cap indices. In one line: a **risk reducer** —
strong protection in bears and crashes, an opportunity cost in bull/sideways regimes
(negative in the one-sided up-market of 2025), not a return enhancer.
...
## 推广研究 / Extension study

六个预注册的推广方向(离散度、额加权广度、窗口内路径、尾盘广度、长假首日、波动率择时),全部在原协议与 25 次试验的族级校正下证伪;仅证实尾盘存在与早盘正交的信息窗口,且不可货币化。证据与证伪工具箱见 [`extension-study/`](extension-study/)。

Six preregistered extensions (dispersion, amount-weighted breadth, intra-window path,
tail breadth, post-holiday reopening, volatility timing), all falsified under the same
protocol with family-wide correction over 25 trials; the one confirmed proposition —
an orthogonal tail-of-day window exists — does not monetize. Evidence and the
falsification toolkit: [`extension-study/`](extension-study/).

- 中文报告:[`早盘情绪因子推广研究.pdf`](extension-study/早盘情绪因子推广研究.pdf)
- English report: [`Extending-the-Early-Session-Sentiment-Factor.pdf`](extension-study/Extending-the-Early-Session-Sentiment-Factor.pdf)
